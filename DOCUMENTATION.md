...
We use YAML (a practice recommended by Snakemake) to configure both tools and rules. Edit `config.yml` at the root of your analysis to configure as you see fit.

### Config file auto-discovery

Both `python mlip/dataflow.py configure` and Snakemake automatically find the most recently modified `config.yml` in any subdirectory. This means you can typically just run commands without specifying which analysis to use.

To override this behavior:

- For `configure`: use `--analysis <name>` to specify a particular analysis directory
- For Snakemake: use `--configfile <path>` to specify a particular config file

Example:
```
snakemake --configfile old-analysis/config.yml -j 4 all
```
...
The `{analysis}/metadata.tsv` file maps sequencing experiment IDs to samples and replicates. It is generated by running `python mlip/dataflow.py preprocess -a <name> -f <ids.txt>` and must be edited before running `python mlip/dataflow.py configure`.

#### Sequencing experiments vs replicates

A **sequencing experiment** represents one run on the sequencer. Each sequencing experiment produces exactly one FASTQ dataset: a pair of gzipped files (forward reads `_R1_001.fastq.gz` and reverse reads `_R2_001.fastq.gz`). Sequencing experiment IDs follow the convention `{Sample}_{Key}{Number}`, for example `BV_W3_Seq1`, `BV_W3_Seq2`.

A **replicate** is an analytical unit—what you want the pipeline to analyze together. The distinction matters because:

- Sometimes 1 replicate = 1 sequencing experiment (the simple case)
- Sometimes 1 replicate = multiple sequencing experiments (e.g., initial sequencing + resequencing to improve coverage)

The `Replicate` column tells the pipeline which sequencing experiments to merge. For example, if `BV_W3_Seq1` and `BV_W3_Seq2` should be analyzed together as replicate 1, assign both rows `Replicate: 1`.

#### Unique identifiers are critical

Sequencing experiment IDs (BaseSpace IDs) must be unique across your entire analysis. The pipeline extracts the sample name by stripping the key suffix (`_Seq1`, `_Rep2`, etc.) in a **case-insensitive** manner. This was intentional—people often mix `Seq1` and `seq2` when naming experiments, and we didn't want to be too rigid.

However, this means `BV_W3_Rep1` and `BV_W3_rep1` look like the **same** sequencing experiment to the pipeline. If you have resequencing data, use incrementing numbers: `BV_W3_Seq1`, `BV_W3_Seq2`, `BV_W3_Seq3`, `BV_W3_Seq4`—not case variations of the same number.

#### Column reference

| Column | Description |
| ------ | ----------- |
| `SequencingId` | The sequencing experiment ID from BaseSpace (e.g., `Sample_Seq1`). Please try to make this unique. |
| `SampleId` | The biological sample this experiment belongs to. Auto-populated but may need editing. |
| `Replicate` | The replicate number (e.g., `1`, `2`). Must be filled in by the user. |
| `ForwardFastqPath` | Optional. Absolute path to the forward FASTQ file. Leave empty for auto-detection. |
| `ReverseFastqPath` | Optional. Absolute path to the reverse FASTQ file. Leave empty for auto-detection. |

#### FASTQ path overrides

The `ForwardFastqPath` and `ReverseFastqPath` columns are empty by default. In most cases, the pipeline auto-detects FASTQ file locations based on the `SequencingId` and the `data_root_directory` in your config.

However, if you already have data with IDs that collide (like the `Rep1`/`rep1` situation described above), you can use the path columns to explicitly specify which files to use rather than renaming everything in BaseSpace.

Paths should be absolute (or use `~` for your home directory). To find the absolute paths to your FASTQ files, you can use:

```bash
# From your data root directory, find all FASTQ files
find . -type f -name "*.fastq.gz"

# Get the full absolute path for a specific file
readlink -f BV_W3_rep1_S1_L001_R1_001.fastq.gz
```

Example metadata with path overrides:

```
SequencingId	SampleId	Replicate	ForwardFastqPath	ReverseFastqPath
BV_W3_Rep1	bv_w3	1
BV_W3_Rep2	bv_w3	2
BV_W3_rep1	bv_w3	1	/Users/lab/basespace/BV_W3_rep1_S1_L001_R1_001.fastq.gz	/Users/lab/basespace/BV_W3_rep1_S1_L001_R2_001.fastq.gz
BV_W3_rep2	bv_w3	2	~/basespace/BV_W3_rep2_S2_L001_R1_001.fastq.gz	~/basespace/BV_W3_rep2_S2_L001_R2_001.fastq.gz
```

When both path columns are populated for a row, the pipeline uses those paths directly instead of searching. This allows you to handle edge cases without renaming files or restructuring your BaseSpace downloads.
...
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| `filler.fasta`           | “Flattened” reference FASTA for this stage (concatenation of per-segment consensus from the previous remapping). Used as Bowtie2 input. |
| `reference/` (directory) | Bowtie2 index and reference sequence:                                                                                                   |
| – `sequences.fasta`      | Full reference FASTA for the present mapping stage                                                                                      |
| – `sequences.fasta.fai`  | Corresponding index, required by some tools for accesss                                                                                 |
| – `index.*.bt2`          | Bowtie2 index shards                                                                                                                    |
| `bowtie2-stdout.txt`     | Standard output log from Bowtie2 (mapping reads to `reference/sequences.fasta`).                                                        |
| `bowtie2-stderr.txt`     | Standard error log from Bowtie2.                                                                                                        |
| `samtools-stdout.txt`    | Combined stdout from Samtools steps (view → sort → index → mpileup).                                                                    |
| `samtools-stderr.txt`    | Combined stderr from Samtools.                                                                                                          |
| `sorted.bam.bai`         | Index file for the (temporary) `sorted.bam` produced by Samtools.                                                                       |
| `coverage.bedGraph`      | Whole‐genome coverage in bedGraph format (output of `bedtools genomecov -bga`).                                                         |
| `coverage.tsv`           | Tab‐delimited version of `coverage.bedGraph` with header (`segment start end coverage`).                                                |
| `coverage-report.tsv`    | Per‐segment coverage summary (bucketed counts) generated by `compute_coverage_categories_io`.                                           |
| `varscan.vcf`            | Raw VCF produced by VarScan (`mpileup2snp`).                                                                                            |
| `varscan.tsv`            | Same VCF as above but with all “##” header lines removed.                                                                               |
| `varscan.vcf.gz`         | BGZF‐compressed version of `varscan.vcf`.                                                                                               |
| `varscan.vcf.gz.tbi`     | Tabix index for `varscan.vcf.gz`.                                                                                                       |
| `varscan-stderr.txt`     | Standard error from the VarScan SNP‐calling step.                                                                                       |
| `varscan-annotated.tsv`  | Annotated variant table (gene, AA change, synonymous/nonsynonymous) generated by `annotate_varscan`.                                    |
...
| File                  | Description                                                                                               |
| --------------------- | --------------------------------------------------------------------------------------------------------- |
| `reference.fasta`     | Segment‐specific reference sequence (extracted from `reference/sequences.fasta`).                         |
| `reference.fasta.fai` | FASTA index for `reference.fasta` (created by `samtools faidx`).                                          |
| `segment.bam`         | BAM of reads mapped to this segment (extracted from the full BAM by `samtools view`).                     |
| `segment.bam.bai`     | Index for `segment.bam`.                                                                                  |
| `ivar.fa`             | Consensus FASTA called by `ivar consensus` (using the segment‐specific pileup).                           |
| `ivar.qual.txt`       | Per‐base quality scores from `ivar consensus`.                                                            |
| `consensus.fasta`     | Final consensus FASTA: a header line plus the nucleotide sequence from `ivar.fa`.                         |
| `unaligned.fasta`     | Combined FASTA of the original reference for this segment and the new consensus (used as input to MAFFT). |
| `aligned.fasta`       | MAFFT alignment of `unaligned.fasta` (reference vs. consensus).                                           |
| `samtools.fasta`      | Secondary consensus produced by `samtools consensus` (`--mode simple`) for comparison/validation.         |
