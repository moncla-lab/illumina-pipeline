...
The `{analysis}/metadata.tsv` file maps sequencing experiment IDs to samples and replicates. It is generated by running `python mlip/dataflow.py preprocess --analysis <name> -f <ids.txt>` and must be edited before running `python mlip/dataflow.py configure`.
